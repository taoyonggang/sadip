...
cd ~/git/googletest-distribution/ && mkdir build && cd build/ && cmake .. &&  make -j$(nproc) &&  make install
...
cd ~/git &&mkdir Fast-CDR/build && cd Fast-CDR/build && git checkout v1.1.0 && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local &&   cmake --build . --parallel $(nproc)  --target install
cd ~/git &&git clone https://github.com/eProsima/Fast-DDS.git
cd ~/git &&mkdir Fast-DDS/build && cd Fast-DDS/build && git checkout v2.11.2&& cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DFASTDDS_STATISTICS=OFF -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local &&  cmake --build . --parallel $(nproc)  --target install
