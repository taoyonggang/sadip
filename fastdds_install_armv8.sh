...
apt update &&  apt install -y software-properties-common  git cmake libasio-dev libtinyxml2-dev libssl-dev libp11-dev libengine-pkcs11-openssl softhsm2 libengine-pkcs11-openssl  
apt install -y libcurl4-openssl-dev libiodbc2 libiodbc2-dev libmysqlclient-dev libpq-dev  libcurl4-openssl-dev p11-kit libyaml-cpp-dev liblz4-dev libzstd-dev uuid-dev 
...
cd ~/git/googletest-distribution/ && mkdir build && cd build/ && cmake .. &&  make -j8 &&  make install
...
&& cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=ON&&cmake --build . --target install
cd ~/git && git clone https://github.com/eProsima/Fast-CDR.git
cd ~/git &&mkdir Fast-CDR/build && cd Fast-CDR/build && git checkout v1.1.0 && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local &&   cmake --build . --target install
cd ~/git &&git clone https://github.com/eProsima/Fast-DDS.git
cd ~/git &&mkdir Fast-DDS/build && cd Fast-DDS/build && git checkout v2.11.3&& cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DFASTDDS_STATISTICS=ON -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local &&  cmake --build . --target install
#cd ~/git &&  git clone https://github.com/eProsima/Fast-DDS-python.git && cd Fast-DDS-python &&git checkout v1.4.0 \
# && mkdir -p fastdds_python/build &&   cd fastdds_python/build && cmake ..    &&  cmake --build . --target install
...
cd protobuf && git checkout  v3.8.0 && git submodule update --init --recursive && mkdir -p cmake/build/ && cd cmake/build/ \
    && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  &&  cmake --build . --target install
...
cd aws-sdk-cpp && git checkout 1.11.327 && mkdir build && cd build && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  \
    ...
    -DBUILD_ONLY="s3" &&  cmake --build . --config=Release &&  cmake --install . --config=Release
...
cd soci && git checkout v4.0.3 && mkdir build && cd build \
    && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=ON -DSOCI_CXX11=ON && cmake --build . --target install

cd ~/git && git clone -b poco-1.12.4-release https://github.com/pocoproject/poco.git
cd poco && mkdir cmake-build && cd cmake-build \
    && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC" -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=ON -DENABLE_CPPPARSER=ON -DENABLE_XML=ON -DENABLE_JSON=ON &&  cmake --build . --target install

#RUN git config --global http.sslverify false
#COPY spdlog ~/git/spdlog 
cd ~/git && git clone https://github.com/gabime/spdlog
cd spdlog && git checkout v1.12.0 && mkdir build && cd build && cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=ON && cmake --build . --target install


#dev_util

cd ~/git && git clone https://github.com/eProsima/dev-utils.git && cd dev-utils && git checkout v0.4.0 && mkdir -p build/cmake_utils && cd build/cmake_utils \
...
cd ~/git &&  git clone https://github.com/eProsima/DDS-Pipe.git && cd DDS-Pipe && git checkout v0.2.0 && mkdir -p build/ddspipe_core && cd build/ddspipe_core \
...
cd ~/git && git clone --recurse-submodules https://github.com/eProsima/DDS-Record-Replay.git  && cd DDS-Record-Replay && git checkout v0.2.0  && mkdir -p build/ddsrecorder_participants \
    && cd build/ddsrecorder_participants && cmake ../../../DDS-Record-Replay/ddsrecorder_participants \
    -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC" -DCMAKE_INSTALL_PREFIX=/usr/local && cmake --build . --target install
...
cd ~/git && cd DDS-Record-Replay && mkdir -p build/ddsrecorder_yaml && cd build/ddsrecorder_yaml \
    ...
    -DCMAKE_INSTALL_PREFIX=/usr/local &&  cmake --build . --target install
...
cd ~/git && git clone https://github.com/eProsima/DDS-Router.git && cd DDS-Router && git checkout v2.0.0 && mkdir -p mkdir build/ddsrouter_core \
...
cd ~/git && cd lz4/build/cmake && mkdir build && cd build && git checkout v1.9.4 \
	&& cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local  &&  cmake --build . --target install
