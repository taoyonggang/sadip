...
cd ~/git &&mkdir Fast-DDS/build && cd Fast-DDS/build && git checkout v2.11.3&& cmake .. -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DFASTDDS_STATISTICS=OFF -DCMAKE_CXX_FLAGS="-fPIC"  -DCMAKE_INSTALL_PREFIX=/usr/local &&  cmake --build . --parallel $(nproc)  --target install
